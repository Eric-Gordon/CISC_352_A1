...
def get_diagonals(play_board, nds, pds):
    ...
    return [sorted(nd.items(), key=lambda x: x[1], reverse=True), sorted(pd.items(), key=lambda x: x[1], reverse=True), n_d_rows, p_d_rows]
